...
export default function AdminEditItems() {
  ...
  const openEdit = (it) => {
    ...
    setSaveError("");
    setImgPreview(typeof it.img === "string" && it.img.startsWith("data:")
      ? it.img
      : (it.img || null));
    setImgFile(null);
  };
  ...
  const removePickedImage = () => {
    ...
    setImgPreview(null);
    if (fileRef.current) fileRef.current.value = "";
    // Keep existing editing.img value; if you want to clear on save, send img: ""
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      setSaving(true);
      // If a new image was chosen, send as base64
      if (imgFile) {
        payload.img = await fileToBase64(imgFile);
      }
      const updated = await api.put(`/admin/menu/${editing.id}`, payload);
